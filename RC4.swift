...
public class RC4 {
	internal var box = count: 256, repeatedValue: 0 as! [Byte]
	internal var x = 0
	internal var y = 0
	...
	public init(key:String) {
		...
		assert(keyData.length == count(key), "Length of key data must match length of key")
		let keyByteArray = UnsafePointer<UInt8>(keUInt8a.bytes)
	...
	public func crypt(data: NSData) -> NSData {
		...
		var out = UnsafeMutablePointer<UInt8>(outData.bytes)
